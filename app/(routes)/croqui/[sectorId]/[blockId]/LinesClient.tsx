...
export default function LinesClient({
  ...
  const submitReview = async (lineId: string) => {
    ...
    if (!rating) {
      setRatingError((prev) => ({ ...prev, [lineId]: "Selecione uma nota de 1 a 5 estrelas" }));
      return;
    }
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{blockName}</h1>
      {blockDescription && <p className="text-gray-600 dark:text-gray-400 mb-4">{blockDescription}</p>}

      {/* Filtro e ordenação (mesmo código) */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Filtrar por grau</label>
        <select
          value={filterGrade}
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setSortBy("grade-asc")} className={`px-3 py-1 rounded text-sm ${sortBy === "grade-asc" ? "bg-blue-500 text-white" : "bg-gray-200 dark:bg-gray-700 dark:text-white"}`}>Grau ▲</button>
        <button onClick={() => setSortBy("grade-desc")} className={`px-3 py-1 rounded text-sm ${sortBy === "grade-desc" ? "bg-blue-500 text-white" : "bg-gray-200 dark:bg-gray-700 dark:text-white"}`}>Grau ▼</button>
        <button onClick={() => setSortBy("name-asc")} className={`px-3 py-1 rounded text-sm ${sortBy === "name-asc" ? "bg-blue-500 text-white" : "bg-gray-200 dark:bg-gray-700 dark:text-white"}`}>Nome A-Z</button>
      </div>
        ...
        {sortedLines.map((line) => {
          ...
          return (
            <div
              key={line.id}
              id={`line-${line.id}`}
              className={`bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900 p-4 transition-all cursor-pointer ${isExpanded ? "ring-2 ring-blue-400" : "hover:shadow-md dark:hover:shadow-gray-900"}`}
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                    <span className="inline-block bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-xs px-2 py-1 rounded">{line.grade}</span>
                    {ratingMap[line.id] && <span className="text-xs text-gray-500 dark:text-gray-400">★ {ratingMap[line.id].toFixed(1)}</span>}
                    {gradeSuggestionMap[line.id] && <span className="text-xs text-gray-500 dark:text-gray-400">Sugestão: {gradeSuggestionMap[line.id]}</span>}
                  </div>
                  {line.description && !isExpanded && <p className="text-gray-600 dark:text-gray-400 text-sm mt-2 line-clamp-2">{line.description}</p>}
                </div>
              </div>
              ...
              {showReviewForm === line.id && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700" onClick={(e) => e.stopPropagation()}>
                  <p className="text-sm font-medium mb-2 dark:text-white">Avalie esta linha</p>
